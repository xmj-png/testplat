...
def operation_book_get_freecharpters(bookid):
    res = select('SELECT t.`free_sections`  FROM audiobook.`t_book` t WHERE t.book_id=' + bookid, 'db_audiobook')
        ...
        for i in range(len(freelist)):
            if '-' in freelist[i]:
                ...
                for j in range(int(end) - int(start) + 1):
                    ...
                    freelist = list(set(freelist) - set(outlist))
                    freelist.sort()
        return freelist
    elif '-' in rlt:
        freelist1 = []
        ...
        for i in range(len(chargelist)):
            if '-' in chargelist[i]:
                ...
                for j in range(int(end) - int(start) + 1):
                    ...
                    chargelist = list(set(chargelist) - set(outlist))
                    chargelist.sort(reverse=False)
        # print(endlist)
...
def operation_book_get_unbuyedcharpters_all(bookid, token):
    sectioncount = select('SELECT t.`section` FROM audiobook.`t_book` t  WHERE t.`book_id`=' + bookid,
                                 'db_audiobook')
    ...
    re_list = list(set(all_list) - set(operation_book_get_buyedcharpters_all(bookid, token)) - set(operation_book_get_freecharpters(bookid)))
    re_list.sort()
    print('未购章节>>>>>>>>>>>>>>>>>>>',re_list)
    msg = 'no section'
    if re_list:
...
if __name__=='__main__':
    # edit_book_info_by_dict('39720',{'bookName':'机破星河001'})
    operation_book_get_buyedcharpters_all('56444559','Y_obJCdP5ejP1SZKF6GjFg**_32JSsLWj_m5NWfMKEPNwECA4GSBpynUF')