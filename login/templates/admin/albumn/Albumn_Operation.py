...
def get_album_info_dict(album_id):
    ...
    :return: 返回书籍信息
    """
    r = httputils.getadmin(getAdminName('albumInfo'),
                           {'id': str(album_id)},
                           login_admin(),
    ...
    json_res['status']=res['data']['status']
    json_res['waitOffline']=res['data']['waitOffline']
...
def edit_album_info_by_dict(album_id, modifydatas):
    ...
    for k, v in modifydatas.items():
        json_res[k] = v
    r = httputils.postadmin(getAdminName("albumEdit"),
                            json_res,
                            login_admin(),
                            getcurrentPath('AlbumEdit'))
    res = loads(r.text)
...
if __name__=="__main__":
    edit_album_info_by_dict(410125,{'description':'00000000000001'})
