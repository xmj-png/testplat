...
def buy_book_utils(token, bookid: str, items: list, opType: str, ptype: str, useTicketsType: str, useTickets=''):
    ...
    print('购买信息：',dict_result)
    print('购买最终结果（懒人币购买)：', dict_result.get('data'))
    # if dict_result.get('status') == 0:
    #     print('status类型000000000000', type(dict_result.get('status')))
    #     return dict_result.get('data').get('orderNo')
    # elif dict_result.get('status') == 6:
    #     print('status类型6666666666666',type(dict_result.get('status')))
    return dict_result.get('status')
