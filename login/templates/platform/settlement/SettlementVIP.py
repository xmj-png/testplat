...
from login.templates.platform.settlement.get_CurrentTime import month_days, getCurrentTime
...
class SettlementVIP():
    ...
    def adjustPercent_sql_record(self):
        ...
        #查询会员总净收入记录(测试环境用的p_member_order_item表，预发布环境用的test_member_order_item表）
        base_billing_amount=select_billing("SELECT sum(base_billing_amount) from p_member_order_item pmoi where start_time BETWEEN '%s' and '%s' and status=0 and base_billing_amount>0;"%(start_time,end_time))
        # 某个月所有等级的记录
        grade_record = select_billing('''SELECT a.*,grade.award_multiple,grade.need_reward from (
                                      ...
                                      GROUP BY grade_id ) a left JOIN p_member_grade grade on id = a.grade_id ;'''%(self.settlement_month))
        # 某个月参与奖励的等级记录
        award_grade_record = select_billing('''SELECT a.*,grade.award_multiple,grade.need_reward from (
                                                    SELECT (SELECT grade_id from p_member_grade_book pmgb where entity_id = pmpm.entity_id and product_type = pmpm.product_type and status=1 ) as grade_id,SUM(pmpm.play_count ) from p_member_play_month pmpm where 
                                                    `month` = %s and status=0
                                                    GROUP BY grade_id ) a left JOIN p_member_grade grade on id = a.grade_id where grade.need_reward=1;'''%(self.settlement_month))
        # 某个月总有效播放次数记录
        all_play_count = select_billing("SELECT `month`,sum(play_count) from p_member_play_month where `status`=0 and `month` =%s;"%(self.settlement_month))
    ...
    def settlementAmount_sql_record(self,product_type):
        ...
        print(end_time)
        #查询书籍当月的有效播放次数记录
        book_playCount_record=select_billing("select * from p_member_play_month pmpm where entity_id=%s and product_type=%s and `month`='%s'  and status=0"
                                          %(self.entity_id,product_type,self.settlement_month))
        ...
        #查询书籍所在等级的奖励倍数
        award_multiple_record=select_billing('''select award_multiple from p_member_grade pmg where grade_type = 1 and id in (select grade_id from p_member_grade_book pmgb where entity_id=%s and product_type=%s and status=1)'''
                                            %(self.entity_id,product_type))
        if award_multiple_record:
            award_multiple=award_multiple_record[0]['award_multiple']
        else:
            award_multiple=0
        # 查询书籍所在等级的当月有效播放次数记录
        grade_id_record = select_billing("select grade_id from p_member_grade_book pmgb where entity_id=%s and product_type=%s and status=1;" % (self.entity_id, product_type))
        if grade_id_record:
            grade_id = grade_id_record[0]['grade_id']
        else:
            grade_id=0
        grade_playCount_record = select_billing('''SELECT `month`,sum(play_count) from p_member_play_month where `month`='%s'  and product_type=%s and entity_id in 
                                                (SELECT entity_id from p_member_grade_book where grade_id=%s 
                                                 and `status`=1); '''
                                                % (self.settlement_month, product_type, grade_id))
        grade_playCount=grade_playCount_record[0]['sum(play_count)']
        # 查询书籍所在等级的公司补贴级别系数记录
        reward_factor_record = select_billing('''select reward_factor from p_member_grade pmg where grade_type = 2 and id in (select company_subsidy_grade_id from p_member_grade_book pmgb where entity_id=%s and product_type=%s and status=1)'''
                                    % (self.entity_id, product_type))
        ...
        # 查询会员总净收入记录
        platform_base_billing_amount_record = select_billing("SELECT product_type,sum(base_billing_amount) from p_member_order_item pmoi where start_time BETWEEN '%s' and '%s' and product_type=%s and status=0 and base_billing_amount>0;"
                                             % (start_time, end_time,product_type))
        ...
        platform_playCount_awardMultiple=select_billing('''SELECT a.*,grade.award_multiple,grade.need_reward from (
                                                        ...
                                                        GROUP BY grade_id 
                                                        ) a left JOIN p_member_grade grade on id = a.grade_id ;'''%(self.settlement_month,product_type,))
        # 查询合作方的VIP合作业务
        partner_records = select_billing("SELECT * from p_partner_service where partner_id='%s' and service_type=6 ORDER BY id desc LIMIT 1" % (self.partner_id))
        if partner_records:
            partner_record=partner_records
        else:
            partner_record=0
        #查询主播合作方VIP分成资源记录
        Anch_partner_record = select_billing("select * from p_share_fee_resource where res_id = '%s' and share_fee_type = 3 order by create_time desc limit 1;"
                                            % (self.entity_id))
        if Anch_partner_record:
            ...
            Anch_partner_rate=0
        #公司补贴百分比
        company_subsidy_percent_record=select_billing("select * from p_config_server pcs where config_key ='COMPANY_SUBSIDY_PERCENT';")
...
class Channel_Settlement():
    ...
    def channel_vip_platform_settlement_amount(self,product_type=1):
        ...
        #当前月该渠道合作方所有的会员订单（测试环境用的p_member_order_item表，预发布环境用的test_member_order_item表）
        member_order_item=select_billing('''SELECT * from p_member_order_item pmoi 
                                            where product_type=%s and channel_partner_id =%s and create_time BETWEEN '%s' and '%s'and status=0 and channel_amount >0;'''%(product_type,self.partner_id,start_time,end_time))
...
if __name__=='__main__':
    #版权/主播合作方 分别传入结算月份，书籍id，合作方id,平台（1懒人，2芽芽）以及合作方当月分成天数占比
    SettlementVIP(202009,930,1641,1).platform_book_settlement_amount(1)
    ...
    # Channel_Settlement(202009,1500).channel_vip_platform_settlement_amount()
