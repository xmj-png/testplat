...
from decimal import *
from login.templates.platform.settlement.get_CurrentTime import getCurrentTime, month_days
import math
...
class Settlement(object):
    ...
    def settlement_lr_yaya(self,product_type=1):
        ...
        partner_record = select_billing("SELECT * from billing.p_partner_service where partner_id='%s' and service_type=%s ORDER BY id desc LIMIT 1" % (self.partner_id,self.sp_type))
        if partner_record:
            ...
            if cooperator_type == 1 or cooperator_type == 2:
                ...
                if self.sp_type == 2 or self.sp_type == 5:
                    Anch_partner_record = select_billing("select * from p_share_fee_resource where res_id = '%s' and share_fee_type = 2 order by create_time desc limit 1;" % (self.entity_id))
                    ...
                    Anch_partner_record = select_billing("select * from p_share_fee_resource where res_id = '%s' and share_fee_type = 1 order by create_time desc limit 1;" % (self.entity_id))
                    partner_rate = Anch_partner_record[0]['share_fee_ratio']  # 主播合作方分成比例
                else:
                    pass
        else:
        ...
        lr_cp_daily_info = select_billing(''' SELECT * FROM test_resource_daily_billing
                                          ...
                                          % (self.entity_id, self.partner_id, self.sp_type, product_type,start_time,end_time), )
        if lr_cp_daily_info:
            ...
            for day in lr_cp_daily_info:
                ...
                #测试环境切换成这个sql
                # pay_rate=select_billing("SELECT * from p_billing_records_commission where id='%s';" %(commission_id)) #根据commission_id查询相对应的手续费明细
                #预发布切换成这个sql
                pay_rate=select_billing("SELECT * from test_billing_records_commission where id='%s';" %(commission_id)) #根据commission_id查询相对应的手续费明细
                # 统计份额外支付手续费
...
if __name__=="__main__":
    ...
    Settlement(202010,416001,1792,2).settlement_partner()
