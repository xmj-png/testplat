# coding=gbk
from login.templates.platform.common.operate_mysql import select_billing, select_platform


def releaseChannelName():
    '''生成发布渠道的简称和全称'''
    channel_record = select_platform("select * from platform.t_copyright_channel order by id desc limit 1;")
...
def divideChannelName():
    '''生成分成渠道的名称'''
    channel_record = select_billing("select * from billing.p_share_fee_channel order by id desc limit 1;")
    channel_id = channel_record[0]['id']
    channel_id = str(channel_id)
    print(channel_id)
    cp_id_lenth = len(channel_id)
