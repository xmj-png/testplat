...
from login.templates.platform.common import Create_ChannelName
...
def add_ReleaseChannel(systemPlatform=2):
    ...
    if json.loads(r.text)['status'] == 0:
        ReleaseChannel_record = select_platform("select * from platform.t_copyright_channel order by id desc limit 1;")  # 查询添加后的发布渠道记录
...
def add_DivideChannel():
    ...
    if json.loads(r.text)['status'] == 0:
        DivideChannel_record = select_billing("select * from billing.p_share_fee_channel where product_type=1 order by id desc limit 1;")  # 查询添加后的分成渠道记录
        sleep(3)
