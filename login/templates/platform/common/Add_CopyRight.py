...
from login.templates.platform.common import Create_CopyrightName
...
def add_AudioBookCopyright():
    ...
    if json.loads(r.text)['status']==0:
        cp_record = select_platform("select id,full_name from platform.t_copyright WHERE full_name like'%版权%' order by id desc limit 1;") #查询添加后的版权记录
...
def add_ReadBookCopyright():
    ...
    if json.loads(r.text)['status'] == 0:
        cp_record = select_readbook("select * from readbook.rb_partner_ext WHERE code='org_name' and code_value like'懒人听书%' order by partner_id desc limit 1;")  # 查询添加后的版权记录
        sleep(3)
...
def add_ComicCopyright():
    ...
    if json.loads(r.text)['status'] == 0:
        cp_record = select("select * from yyting_partdb.c_comic_copyright  order by id desc limit 1;","db_yyting_partdb")  # 查询添加后的版权记录
        cp_id = cp_record[0]['id']  # 获取版权id
        print(cp_id)
