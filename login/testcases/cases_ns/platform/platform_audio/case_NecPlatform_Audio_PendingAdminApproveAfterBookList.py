...
class case_NecPlatform_Audio_Pending_Admin_Approve_After_BookList(unittest.TestCase):
    def setUp(self):
        warnings.simplefilter('ignore', ResourceWarning) #忽略ResourceWarning提示

    def test_necplatform_audio_pending_admin_approve_after_book_list(self):
        """待懒人复审书籍（后期）列表"""
        # 登录并获取token
