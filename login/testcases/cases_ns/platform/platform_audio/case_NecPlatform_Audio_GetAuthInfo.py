...
class case_NecPlatform_Audio_GetAuthInfo(unittest.TestCase):
    def setUp(self):
        warnings.simplefilter('ignore', ResourceWarning) #忽略ResourceWarning提示

    def test_necplatform_audio_get_auth_info(self):
        """获取听书号的实名认证信息"""
        # 登录并获取token
        platform_token = login_platform()
        # 获取platform接口url
        getAuthInfo=getPlatformName('getAuthInfo')
        #入参
        params={}
        #调用接口
        res=httputils.get_platforms(getAuthInfo,params,platform_token)
        #将json格式的字符串转换成字典
