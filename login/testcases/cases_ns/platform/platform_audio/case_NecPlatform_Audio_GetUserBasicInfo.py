...
class case_NecPlatform_Audio_GetUserBasicInfo(unittest.TestCase):
    def setUp(self):
        warnings.simplefilter('ignore', ResourceWarning) #忽略ResourceWarning提示

    def test_necplatform_audio_get_user_basic_info(self):
        """获取听书号-账号信息页账号信息"""
        # 登录并获取token
        platform_token = login_platform()
        # 获取platform接口url
        getUserBasicInfo=getPlatformName('getUserBasicInfo')
        #入参
        params={}
        #调用接口
        res=httputils.get_platforms(getUserBasicInfo,params,platform_token)
        #将json格式的字符串转换成字典
