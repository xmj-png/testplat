...
class case_NecPlatform_Billing_Get_Billing_Info_Detail(unittest.TestCase):
    def setUp(self):
        warnings.simplefilter('ignore', ResourceWarning) #忽略ResourceWarning提示

    def test_necplatform_billing_get_billing_info_detail(self):
        """获取听书号-个人中心结算信息"""
        # 登录并获取token
        platform_token = login_platform()
        # 获取platform接口url
        getBillingInfoDetail=getPlatformName('getBillingInfoDetail')
        #入参
        params={}
        #调用接口
        res=httputils.get_platforms(getBillingInfoDetail,params,platform_token)
        #将json格式的字符串转换成字典
