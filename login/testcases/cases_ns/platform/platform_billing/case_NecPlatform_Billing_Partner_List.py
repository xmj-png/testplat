...
class case_Platform_Billing_Partner_List(unittest.TestCase):

    def test_platform_billing_partner_list(self):
        """分成合作方列表页"""
        # 登录并获取token
    ...
    def test_platform_channel_partner_list(self):
        """分成合作方(渠道）列表页"""
        # 登录并获取token
        admin_token = login_admin()
        # 获取admin接口url
        ...
        #校验接口请求是否成功
        self.assertTrue(len(r['list'])>0 and r['status']==0,msg='测试失败！！！')
    def test_platform_copyright_partner_list(self):
        """分成合作方(版权）列表页"""
        # 登录并获取token
        admin_token = login_admin()
        # 获取admin接口url
        ...
        #校验接口请求是否成功
        self.assertTrue(len(r['list'])>0 and r['status']==0,msg='测试失败！！！')
    def test_platform_anchor_partner_list(self):
        """分成合作方(主播）列表页"""
        # 登录并获取token
        admin_token = login_admin()
        # 获取admin接口url
