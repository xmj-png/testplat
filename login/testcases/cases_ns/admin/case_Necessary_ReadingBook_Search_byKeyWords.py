...
class case_Necessary_Admin_ReadingBook_Search_byKeyWords(unittest.TestCase):

    def test_necessary_admin_readingbook_search_by_keywords(self):
        """admin后台阅读书籍搜索"""
        # 登录并获取token
        admin_token = login_admin()
        # 获取书籍
        random_int = random.randint(0, 100)
        book = dbutil.select('SELECT id,book_name FROM `t_read_book` WHERE state=1 LIMIT %d,1' % random_int,'db_audiobook')
        print('获取的book', book)
