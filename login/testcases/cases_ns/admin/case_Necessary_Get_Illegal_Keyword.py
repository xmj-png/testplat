...
# @Author : yuhao
# @Site : 
# @File : case_Necessary_Get_Illegal_Keyword.py
# @Software: PyCharm

"""
后台进入审核管理-》敏感词库：筛选非法关键词
"""
...
class case_Necessary_Admin_Get_Illegal_Keyword(unittest.TestCase):

    def test_necessary_admin_get_illegal_keyword(self):
        """admin敏感词库：筛选非法关键词，数据正常返回"""
        # 登录并获取token
        admin_token = login_admin()
        #获取当前时间,请求拼接
        t = time.time()
        ms = int(round(t * 1000))
        #type=2&pageNum=1&_=1603785116033
        data = {
            "type": "2",
            "pageNum": "1",
            "_": ms
