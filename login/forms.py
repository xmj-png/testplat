...
class phone_register(forms.Form):
    """手机号注册"""
    host_name = (
        ('moon', '月亮'),
        ('mars', '火星'),
...
class ReportUtils(forms.Form):
    ...
    api_choice = (
        ...
        ('platform-billing-web', 'platform-billing-web'),
        ('platform-audio-admin', 'platform-audio-admin'),
        ('platform-audio-web', 'platform-audio-web'),
    )
    envId = forms.ChoiceField(label="测试环境", choices=host_name)
    test_type = forms.ChoiceField(label='用例执行类型', choices=test_choice)
...
class AddCase(forms.Form):
    case_name_ch = forms.CharField(label="用例名-中文", max_length=64,
                                   widget=forms.TextInput(attrs={'class': 'form-control'}))
    case_type_choice = (('app','app用例'),('admin','admin用例'),('platform','platform用例'))
    case_type = forms.ChoiceField(label="用例类型",choices=case_type_choice,
                                   widget=forms.Select())
...
class Account_Charge(forms.Form):
    ...
    channel_name = (
        ...
        ('xiaomi', 'xiaomi'),
        ('vivo', 'vivo'),
        ('qq_appcenter', 'qq_appcenter'),
        ('oppo_nearme', 'oppo_nearme'),
        ('oppo_pay', 'oppo_pay'),
        ('samsung', 'samsung'),
    )
    pay_type_name = (
    ...
    user_id = forms.CharField(label='用户ID', max_length=64,
                              widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': "懒人ID"}))
    amount = forms.CharField(label='懒人币数量', max_length=64,
                             widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': "懒人币数量"}))
    pay_type = forms.ChoiceField(label='支付方式', choices=pay_type_name,
                               widget=forms.RadioSelect(attrs={'required': 'required','class': 'radioset'}))
    channel = forms.ChoiceField(label='用户渠道', choices=channel_name,
                              widget=forms.RadioSelect(attrs={'required': 'required','class': 'radioset'}))
