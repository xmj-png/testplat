...
from login.templates.platform.add_Partner.Add_AnchorPartner import add_AnchorPartner
...
def add_anchor_partner(request):
    ...
    if request.method:
        ...
        if anchor_form.is_valid():
            ...
            business_type = anchor_form.cleaned_data.get('business_type')
            partnerType = anchor_form.cleaned_data.get('partnerType')
            print('数据类型：', type(business_type))
            init_configs(host_name)
            anchor_list = []
            for anchor in business_type:
